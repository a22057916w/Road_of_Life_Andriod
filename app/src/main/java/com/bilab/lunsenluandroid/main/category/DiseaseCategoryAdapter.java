...
public class DiseaseCategoryAdapter extends RecyclerView.Adapter<DiseaseCategoryAdapter.ViewHolder>{
    private final Context context;
    private final ArrayList<DiseaseCategoryModel> diseaseCategoryModelList;

    // Constructor
    public DiseaseCategoryAdapter(Context context, ArrayList<DiseaseCategoryModel> diseaseCategoryModelList) {
    ...
    @NonNull
    @Override
    public DiseaseCategoryAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        // to inflate the layout for each item of recycler view.
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_disease_regular, parent, false);

        return new DiseaseCategoryAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull DiseaseCategoryAdapter.ViewHolder holder, int position) {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private Pair<String, Integer> getDiseaseCategory() {
            ...
            if(disease_category.contains("糖尿病腎病變"))
                return new Pair<>(Constant.DKD, R.drawable.ic_kidney_or_dkd);
